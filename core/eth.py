...
def start():
    ...
    while (confirm_acceptable_block_num > current_blocknum):
        ...
        headers = {
            'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.102 Safari/537.36'}
        block = (requests.get(config["ethereum_api_getBlockByNumber_url"] + current_blocknum_hex +
                              '&boolean=true&apikey=' + etherscan_apikey, headers=headers)).json()['result']
        ...
        try:
            ...
            Ethereum_Transaction.objects.filter(
        ...
        if succsses :
            ...
            block = block_save(block_height=str(
                current_blocknum), system='ethereum')
